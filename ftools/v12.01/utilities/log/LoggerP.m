...
methods (Access = public)
    ...
    function p = LoggerP(fname)
        ...
        [p.dir,p.basename,p.ext] = fileparts(p.fullpath);

        % p.logname = [ p.basename '-' p.stamp '_id' RUN_ID ];
        p.logname = [ p.basename datestr(now, '_yy_mm_dd_') 'Time' datestr(now, 'HHMMSS') '_id' RUN_ID ];
        p.logpath = [ cd filesep 'results' filesep p.logname ];
        p.file = @(fname) [p.logpath filesep fname];
