function [ret] = pcz_num2str_v2(qty,args)
arguments
    qty
    args.format = '%8.3f';
    args.del1 = ' , ';
...
for i = 1:numel(qty)
    ...
    label = sprintf(args.label);
    if strfind(label, args.inputname)
        label = strrep(label, args.inputname, iname);
    end
    ...
    if isempty(args.var)
        ...
        str = pcz_num2str_fixed(args.var,args.format, args.del1, args.del2, args.pref, ...
            [label args.beginning], args.ending);
    end
